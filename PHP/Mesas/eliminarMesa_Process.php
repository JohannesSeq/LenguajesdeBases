...
if (isset($_POST['id'])) {
    ...
    $query = "BEGIN 
                :resultado := EJECUTAR_BORRADO_MESA(:P_ID_MESA, :P_TIPO, :P_COMENTARIO); 
              END;";

    $stmt = oci_parse($conn, $query);

    $comentario = 'Eliminando mesa desde proceso PHP';
    $resultado = '';

    oci_bind_by_name($stmt, ":P_ID_MESA", $id);
    ...
    echo json_encode([
        ...
        'mensaje' => match($resultado) {
            'BORRADO_LOGICO' => 'Mesa marcada como inactiva.',
            'BORRADO_FISICO' => 'Mesa eliminada permanentemente.',
            'REFERENCIAS_ACTIVAS' => 'No se puede eliminar: la mesa tiene reservaciones.',
