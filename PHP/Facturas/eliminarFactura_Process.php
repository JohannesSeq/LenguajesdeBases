...
if (isset($_POST['id'])) {
    ...
    $query = "BEGIN 
                :resultado := EJECUTAR_BORRADO_FACTURA(:P_ID_FACTURA, :P_TIPO, :P_COMENTARIO); 
              END;";

    $stmt = oci_parse($conn, $query);

    $comentario = 'Eliminando factura desde proceso PHP';
    $resultado = '';

    oci_bind_by_name($stmt, ":P_ID_FACTURA", $id);
    ...
    echo json_encode([
        ...
        'mensaje' => match($resultado) {
            'BORRADO_LOGICO' => 'Factura marcada como inactiva.',
            'BORRADO_FISICO' => 'Factura eliminada permanentemente.',
            'NO_EXISTE_FACTURA' => 'La factura no existe o ya fue eliminada.',
