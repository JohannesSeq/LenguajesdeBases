...
-------------------- Restaurado logico ----------------------------------------
CREATE OR REPLACE PROCEDURE RESTAURAR_SIN_ID(
    P_TABLA IN VARCHAR,
    P_ID IN NUMBER,
    P_COMENTARIO IN VARCHAR
)
...
CREATE OR REPLACE PROCEDURE RESTAURAR(
    P_ESTADO_ID IN VARCHAR
)
AS
BEGIN
    UPDATE ESTADOS
    SET ESTADO = 'Activo',
        FECHA_CAMBIO = CURRENT_DATE
    WHERE ID_ESTADO = P_ESTADO_ID;

    COMMIT;
END;
/

------------------------- Obtener entradas inactivas -------------------------------

CREATE OR REPLACE VIEW ESTADOS_INACTIVOS 
AS
    select ID_ESTADO, TABLA_ENTRADA, ESTADO FROM ESTADOS 
    PARTITION(P_INACTIVOS);
/

CREATE OR REPLACE PROCEDURE OBTENER_ESTADOS_INACTIVOS(
    P_CURSOR OUT SYS_REFCURSOR
)
AS
BEGIN
    OPEN P_CURSOR FOR SELECT * FROM ESTADOS_INACTIVOS;
END;
/
